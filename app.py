...
# create lightweight session options (VERY IMPORTANT)
...
try:
    failure_session = ort.InferenceSession(
        os.path.join(MODEL_DIR, "multi_failure_model.onnx"),
        sess_options=so,
...
try:
    rul_rf_session = ort.InferenceSession(
        os.path.join(MODEL_DIR, "rul_rf_model.onnx"),
        sess_options=so,
        providers=["CPUExecutionProvider"]
    )

    rul_gb_session = ort.InferenceSession(
        os.path.join(MODEL_DIR, "rul_gb_model.onnx"),
        sess_options=so,
        providers=["CPUExecutionProvider"]
    )
